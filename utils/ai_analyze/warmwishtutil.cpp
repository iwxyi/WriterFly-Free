...
QString WarmWishUtil::getWarmWish(QString path, QString dir)
{
    QString all = readTextFileWithFolder(path, dir);
    QStringList wishs = getPromptWish(all);
    ...
    // 获取随机祝福
    srand(time(0));
    int r = qrand() % (wishs.size()*3); // 祝福语概率，三分之一
    if (r >= wishs.size())
        return "";
    return wishs.at(r);
}
