...
TipCard::TipCard(QWidget *parent, NotificationEntry *noti)
    : ThreeDimenButton(parent), noti(noti), enter_timestamp(0)
{
    ...
    aop_w = parentWidget()->width() / AOPER;
    ...
    QHBoxLayout* margin_hlayout = new QHBoxLayout(this);
    {
        ...
        margin_hlayout->addSpacing(aop_w);
    ...
    connect(this, &ThreeDimenButton::clicked, [=]{
        if (is_closing) return ;
        emit noti->signalCardClicked(); // 发出通知的局部接收
        emit signalCardClicked(noti);   // 全局接收

        // 没有信息展示内容，并且只有一个按钮，相当于点击了第一个按钮
        if (noti->desc.isEmpty() && !noti->btn1.isEmpty() && noti->btn2.isEmpty() && noti->btn3.isEmpty())
        {
            emit noti->signalBtnClicked(1);
            emit signalButton1Clicked(noti);
        }
...
void TipCard::leaveEvent(QEvent *event)
{
    ...
    if (!has_leaved && enter_timestamp + 300 < getTimestamp()) // 300 毫秒内进入又离开，视作划过，不进行任何操作
        has_leaved = true;
}
